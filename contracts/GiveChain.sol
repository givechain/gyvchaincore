...
contract GiveCoin is ERC20 , ReentrancyGuard{
    ...
    event CampaignCreated (uint indexed campaignId, uint campaignAmount);
    
    event CampaignApproved (uint indexed campaignId, uint campaignAmount);
    
    event Donated(uint indexed campaignId, uint pendingAmount);
    ...
    function buyTokens() public nonReentrant payable {
        ...
        require (balanceOf(address(this)) > tokenAmount, "Not enough tokens in the contract") ;
    ...
    function closeCampaign(uint256 _index) public payable onlyAdmins {
        ...
        require( campaign.campaignStatus == CAMPAIGN_STATE.OPEN, "Campaign Cant be closed from this status");
